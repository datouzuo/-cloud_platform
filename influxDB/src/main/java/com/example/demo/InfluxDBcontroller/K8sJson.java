...
all.setRequest(df.format(dou));
	...
	public List<NodeCpu> nodeCpuCapacity() throws ParseException, IOException {
		...
		List<NodeCpu> list = new LinkedList<>();
   		...
   		//1）查询allocation的值
   	influxDB = InfluxDBFactory.connect(infulxUrl);
   		Query query = new Query("SELECT mean(\"value\") FROM \"cpu/usage_rate\"  WHERE \"type\"='node'  AND  time > now() - 63m  AND time < now()-3m GROUP BY time(5m)", "k8s");
   		QueryResult queryResult = influxDB.query(query);
   		List<Result> results = queryResult.getResults();
   		DecimalFormat df1 = new DecimalFormat("0.0000");
...
return time;
	...
	public List<NodeMemory> nodeMemoryCapacity() throws ParseException, IOException {
		...
		List<NodeMemory> list = new LinkedList<>();
   		...
   		//1）查询allocation的值
   		influxDB = InfluxDBFactory.connect(infulxUrl);
   		Query query = new Query("SELECT mean(\"value\") FROM \"memory/usage\"  WHERE \"type\"='node' AND time > now() - 63m AND time < now()-3m GROUP BY time(5m)", "k8s");
   		QueryResult queryResult = influxDB.query(query);
   		List<Result> results = queryResult.getResults();
   		DecimalFormat df1 = new DecimalFormat("0.0000");
     List<List<Object>> values = results.get(0).getSeries().get(0).getValues();
	...
	//pod的cpu使用情况
		...
		public List<NodeCpu> clusterCpuCapacity() throws Exception {
			...
			List<NodeCpu> list = new LinkedList<>();
	   		...
	   		//1）查询allocation的值
	   		 influxDB = InfluxDBFactory.connect(infulxUrl);
	   		Query query = new Query("SELECT mean(\"value\") FROM \"cpu/usage_rate\"  WHERE \"type\"='pod'  AND  time > now() - 63m AND time < now()-3m GROUP BY time(5m)", "k8s");
	   		QueryResult queryResult = influxDB.query(query);
	   		List<Result> results = queryResult.getResults();
	   		DecimalFormat df1 = new DecimalFormat("0.0000");
	     List<List<Object>> values = results.get(0).getSeries().get(0).getValues();
		...
		public List<NodeMemory> clusterMemoryCapacity() throws ParseException, JsonParseException, JsonMappingException, IOException {
			...
			List<NodeMemory> list = new LinkedList<>();
	   		...
	   		//1）查询allocation的值
	   		influxDB = InfluxDBFactory.connect(infulxUrl);
	   		Query query = new Query("SELECT mean(\"value\") FROM \"memory/usage\"  WHERE \"type\"='pod' AND time > now() - 63m AND time < now()-3m GROUP BY time(5m)", "k8s");
	   		QueryResult queryResult = influxDB.query(query);
	   		List<Result> results = queryResult.getResults();
	   		DecimalFormat df1 = new DecimalFormat("0.0000");
	     List<List<Object>> values = results.get(0).getSeries().get(0).getValues();
